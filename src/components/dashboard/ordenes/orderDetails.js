...
function Orders() {
  return (
    <div className="space-y-4">
        ...
        <div key={product.id} className="bg-white rounded-2xl border border-sofi-100 shadow-sm overflow-hidden">
          ...
          <div className="px-6 py-4 border-b border-sofi-100 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <Link to="/DetallesOrden" className="text-xs font-semibold text-sofi-500 hover:text-sofi-700 transition-colors">
                Ver detalle →
              </Link>
            </div>
          </div>
          ...
          <div className="px-6 py-5 flex gap-5 border-b border-sofi-50">
            <div className="flex-shrink-0 w-20 h-20 rounded-xl overflow-hidden bg-sofi-50 border border-sofi-100">
              <ImageGalery photo={product.imageSrc} />
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="text-sm font-semibold text-sofi-800">{product.name}</h3>
              <p className="text-xs text-sofi-500 mt-1 line-clamp-2">{product.description}</p>
      ...
      {products.length === 0 && (
        <div className="bg-white rounded-2xl border border-sofi-100 shadow-sm px-6 py-16 text-center">
          <p className="text-sofi-400 text-sm">Todavía no tenés pedidos realizados.</p>
          <Link to="/" className="mt-4 inline-block text-sm font-semibold text-sofi-500 hover:text-sofi-700 transition-colors">
            Ir a la tienda →
          </Link>
        </div>
      )}
